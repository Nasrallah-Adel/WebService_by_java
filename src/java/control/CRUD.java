...
public class CRUD {
    ...
    public static int Insertstudent(stud user) {
    ...
    public static List getUsersData() {
        DB.s.beginTransaction();
        Query q = DB.s.createQuery("from stud");
        q.setFirstResult(0);

        List<stud> per = q.list();
        DB.s.getTransaction().commit();
        DB.s.clear();
        return per;
...
//    }
//
//    public static void updateuser_lev(user u, int sum) {
//        DB.s.beginTransaction();
...
//    }
//
    public static boolean is_exist(model.stud user) {
        DB.s.beginTransaction();
        Query q = DB.s.createQuery("from stud where email=:n");
        q.setFirstResult(0);
